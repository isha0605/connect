...
def _fetch_school_certificate(user_email: str, course: str) -> dict:
	"""Looks up a live LMS Certificate on school.frappe.io for this email + course, and upserts a
	local Partner Certificate cache row for it. Throws distinctly on "not found" vs. "couldn't
	reach School" — a network failure must never be silently treated as a missing certificate."""
	client = _get_school_client()

	try:
		matches = client.get_list(
			"LMS Certificate",
			filters=[["member", "=", user_email], ["course", "=", course]],
	...
	if not matches:
		frappe.throw(_("No certificate found for {0} on Frappe School for this course.").format(user_email))
	...
	if existing_name:
		...
		return frappe.db.get_value("Partner Certificate", existing_name, "partner")
...
def create_or_resend(partner: str, user_email: str, course: str) -> dict:
	"""Verifies the certificate exists on Frappe School (live), then dedupes: reuses an existing
	Pending request for the same (partner, email, course) instead of creating a duplicate."""
	existing_link = _fetch_school_certificate(user_email, course)
	if existing_link and existing_link != partner:
		frappe.throw(_("This certificate is already linked to another partner."))
